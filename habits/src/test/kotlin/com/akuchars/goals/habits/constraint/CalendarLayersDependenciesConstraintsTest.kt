package com.akuchars.goals.habits.constraint
...
internal class CalendarLayersDependenciesConstraintsTest {
    ...
    fun validateLayerDependencies(msg: String, fromLayer: String, toLayer: String, excludedClassessToAnalyze: List<Class<*>>, excludedImports: List<Class<*>>) {
        ...
        //then
        assertTrue(illegalImports.isEmpty, "Found illegal dependencies / imports - " + msg + " (in " + illegalImports.count() + " classes) from '" +
                fromLayer + "' layer to '" + toLayer + "' layer: " + illegalImports.toString())
    }

    @Test
    fun `should be all @Configuration class without @Autowired on fields`() {
        getClassesSource()
                .asSequence()
                .filter { it.hasAnnotation(Configuration::class.java) }
                .map { it.name to it.fields }
                .toMap()
                .forEach {
                    assertFalse(it.value.any { it.hasAnnotation(Autowired::class.java) },
                            "Class ${it.key} cannot contain @Autowired")
                }
    }

    private fun getClassesSource(filePathEnding: Array<String> = arrayOf(".kt", ".java")): List<JavaClassSource> {
        return findAllSourceFilesWithPackage("calendar", filePathEnding)
                .asSequence()
                .map(this::parse)
                .filter(JavaSource<*>::isClass)
                .map { it as JavaClassSource }
                .toList()
    }
    ...
    companion object {
        ...
        private fun layersAndExcludes(): List<Arguments> {
            return listOf(
                    ...
                    Arguments.of("UI nie może zawierać DOM", "ui", "domain", listOf<Any>(), listOf<Any>()),
